...
- **Grocery list** — `.grocery-toolbar` contains `#groceryCustomInput`, `.btn-primary` (Add), and `.grocery-clear-btns` wrapper div holding the two clear buttons. The wrapper exists so mobile CSS can give the input more space (via `1fr auto` grid) while keeping clear buttons equal-width (via `flex: 1` inside the wrapper). Each `.grocery-item` wraps its count stepper, shop, and remove buttons in a `.grocery-item-actions` div: on desktop it's a plain flex child at the end of the row, but on mobile the item switches to a two-row grid (text on top, actions underneath) so long ingredient text gets the full card width.
