...
def _parse_html_ingredient(el) -> Optional[str]:
...
def parse_recipe_json(content: str) -> Optional[RecipeCreate]:
    try:
        ...
        return RecipeCreate(
            ...
            recipeInstructions=data.get("recipeInstructions") or data.get("instructions") or "",
...
def parse_recipe_markdown(content: str) -> Optional[RecipeCreate]:
    try:
        ...
        if title_end != -1:
            ...
            description = content[desc_start:desc_end].strip() if desc_end != -1 else content[desc_start:].strip()
        else:
            description = ""
        ...
        if inst_match:
            instructions_text = re.sub(r'^\d+\.\s+', '', inst_match.group(1), flags=re.MULTILINE).strip()
        ...
        img_match = re.search(r'Image(?:\s*URL)?:?\s*(https?://\S+)', content, re.IGNORECASE)
        if img_match:
